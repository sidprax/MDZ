...
const hookName = event.hook_event_name ?? event.hookEventName ?? process.env.MDZ_HOOK_EVENT ?? "Unknown";
const policy = await readPolicy();

if (/PreInvocation|UserPromptSubmit/i.test(hookName)) {
  await handlePrompt(event, policy, hookName);
} else if (/PostToolUse|PostToolUseFailure/i.test(hookName)) {
  await handleToolOutput(event, policy, hookName);
} else if (/Stop|PostInvocation|SessionEnd/i.test(hookName)) {
  await handleStop(event, policy, hookName);
} else {
  await writeHookReport("antigravity-generic", {
    generatedAt: new Date().toISOString(),
    hook: hookName,
    policy,
    eventKeys: Object.keys(event)
  });
  console.error(`MDZ Antigravity hook observed ${hookName}`);
}
...
async function handlePrompt(event, policy, hookName) {
  ...
  await writeHookReport("antigravity-pre-invocation", report);
  await recordLedgerEvent({
    source: { platform: "antigravity", hook: hookName },
    type: "prompt",
    mode: policy.mode,
  ...
  console.error(`MDZ Antigravity prompt hook: ${estimate.tokens} estimated tokens, action=${advisor.action}`);
}
...
async function handleToolOutput(event, policy, hookName) {
  ...
  const report = {
    ...
    delivered: false,
    deliveryReason: "Antigravity native hook is advisory; use mdz_gateway for enforceable replacement.",
    advisor,
    text: renderAdvisorReport(advisor)
  };

  await writeHookReport("antigravity-post-tool-use", report);
  await recordLedgerEvent({
    source: { platform: "antigravity", hook: hookName },
    type: "tool-output",
    mode: policy.mode,
  ...
  console.error(`MDZ Antigravity tool hook: saved=${filtered.metrics.savedTokens} estimated tokens, action=${advisor.action}`);
}
...
async function handleStop(event, policy, hookName) {
  ...
  await writeHookReport("antigravity-stop", report);
  console.error(`MDZ Antigravity stop hook: events=${usage.totals.events}, saved=${usage.totals.savedTokens}`);
}
